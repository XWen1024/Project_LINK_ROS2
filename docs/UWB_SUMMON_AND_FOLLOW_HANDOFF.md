...
- 召唤与跟随共用 `/uwb_navigation/person_navigation` Action：
  - 召唤：只计算并提交一个静态 Nav2 目标，机器人到佩戴者附近约 `1.0 m`
    后完成并停车；提交后只用新观测检查数据新鲜度和到达距离，不再计算人员
    速度或更新方向，不得使用跟随模式的周期刷新取消该目标。
...
第二次 live 验证确认没有再次刷新目标，但提交后的 PDoA 方向跳动触发了共用的
`3.0 m/s` 人员速度门，约 `0.45 s` 后再次安全取消。现在 summon 提交后冻结目标，
后续观测只承担 freshness 和纯测距到达门；人员速度门仅属于 follow。
