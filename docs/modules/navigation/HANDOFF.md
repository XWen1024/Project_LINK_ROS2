...
- 首选现场链路：Orin 的 Jetson USB device-mode `l4tbr0` 地址
  ...
  到原 domain 42 局域网路径。
- Orin 的 `scripts/project_link_env.sh` 也会在 `192.168.55.100` 可达时覆盖
  `console.env` 中残留的 Wi-Fi 参数；独立 Nav2 启动脚本会先停止完整依赖链
  再统一启动。2026-08-21 曾出现雷达服务为 USB profile、扫描服务为 native
  profile 的混合状态，表现为 `/unilidar/cloud` 只有 topic 名而无发布者，
  中控永远卡在“二维雷达扫描”。上述双重保护用于防止复发。
