...
status_cmd="cd '$WORKSPACE' && source scripts/project_link_env.sh && while true; do clear; date; echo; echo 'tmux sessions:'; tmux ls 2>/dev/null || true; echo; echo 'required topics:'; for topic in /odom /odom_lio /scan_accumulated /map /local_costmap/costmap /global_costmap/costmap; do printf '%-32s ' \"\$topic\"; timeout 3 ros2 topic echo --once \"\$topic\" >/dev/null 2>&1 && echo OK || echo MISSING; done; echo; echo 'Point-LIO rate:'; timeout 8 ros2 topic hz /odom_lio 2>&1 | grep -m1 'average rate' || true; echo; echo 'Navigation actions:'; timeout 5 ros2 action list 2>/dev/null | grep -E 'navigate_to_pose|uwb_navigation/person_navigation' || true; echo; echo 'UWB status:'; timeout 3 ros2 topic echo --once /uwb_navigation/status --field data 2>/dev/null || echo 'not running'; echo; echo '/cmd_vel endpoints:'; timeout 5 ros2 topic info /cmd_vel -v 2>&1 | grep -E 'Publisher count|Subscription count|Node name|Endpoint type' || true; echo; echo 'TF map -> base_footprint:'; timeout 8 ros2 run tf2_ros tf2_echo map base_footprint 2>&1 | head -n 14 || true; sleep 5; done"
