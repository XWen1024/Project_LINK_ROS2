...
SESSION="${PROJECT_LINK_TMUX_SESSION:-project_link_nav_prereq}"
...
usage() {
  cat <<EOF
Usage: ./start_nav_prereq_tmux.sh [options]

Start the hardware/odom prerequisites for Nav2, without mapping-mode slam_toolbox.
This starts C63A base serial, Unitree lidar, /scan conversion, robot description,
rf2o, and EKF. It does not start Nav2 and does not publish /cmd_vel.

Options:
  --restart       Stop the tmux session and clean known base/lidar/odom processes first.
  --clean         Clean known base/lidar/odom processes before starting.
...
Environment overrides:
  PROJECT_LINK_TMUX_SESSION   default: project_link_nav_prereq
...
clean_ros_processes() {
  echo "[clean] Stopping known Project LINK base/lidar/odom processes..."
  ...
  pkill -f 'rf2o_ekf_odom.launch.py' || true
...
odom_cmd="cd '$WORKSPACE' && source scripts/project_link_env.sh && $wait_base_cmd && $wait_scan_cmd && ros2 launch turn_on_wheeltec_robot rf2o_ekf_odom.launch.py"
check_cmd="cd '$WORKSPACE' && source scripts/project_link_env.sh && echo 'Mode: Nav2 prerequisites only. Start patrol_nav2.launch.py separately after this is healthy.'; while true; do clear; date; echo; echo 'Nodes:'; ros2 node list 2>/dev/null | sort || true; echo; for topic in /odom /imu/data_raw /PowerVoltage /unilidar/cloud /scan /odom_rf2o /odometry/filtered; do echo \"=== \$topic ===\"; timeout -s INT 4 ros2 topic hz \"\$topic\" 2>&1 | grep -E 'average rate|WARNING|error|does not appear' | tail -n 3 || true; done; echo; echo 'TF odom -> base_footprint:'; timeout -s INT 4 ros2 run tf2_ros tf2_echo odom base_footprint 2>&1 | head -n 14 || true; echo; echo 'No map -> odom is expected here; patrol_nav2 localization owns that later.'; echo 'Ctrl-C stops this monitor only. Use tmux kill-session -t $SESSION to stop all panes.'; sleep 5; done"
